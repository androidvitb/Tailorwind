...
exportCodeButton.addEventListener("click", () => {
  ...
  popup.innerHTML = `
      <div class="bg-white p-6 rounded-lg w-4/5 max-w-screen-lg shadow-lg">
        <h2 class="text-lg font-bold mb-4">Copy the Code</h2>
        <textarea id="codeArea" class="w-full h-48 bg-gray-100 p-2 rounded border border-gray-300" readonly>${cardHTML}</textarea>
        <button id="copyButton" class="mt-4 bg-blue-600 text-white px-4 py-2 rounded w-full">Copy to Clipboard</button>
        <button id="closeButton" class="mt-2 bg-gray-400 text-white px-4 py-2 rounded w-full">Close</button>
      </div>
    `;
